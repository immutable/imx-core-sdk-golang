...
func TestStarkSignerFactory_GenerateLegacyKey_GrindkeyFix(t *testing.T) {
	...
	tests := []test{
		...
		{
			name:          "grindKeyfix hashed more than once",
		...
		{
			name:          "grindKeyfix is backwards compatible",
		...
		{
			name:          "grindKeyfix hashed once is backwards compatible",
		...
		{
			name:          "grindKeyfix hashed more than once is backwards compatible",
		...
		{
			name:          "grindKeyfix hashed once generates correct legacy key",
		...
		{
			name:          "grindKeyfix hashed more than once generates correct legacy key",
		...
		{
			name:          "random eth private key with no index generates correct legacy key",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			signer, err := NewSigner(starkKeyInHex)
			assert.NoError(t, err)
			starkPublicKey := signer.GetPublicKey()
			assert.Equal(t, tt.wantPublicKey, starkPublicKey)
		})
	}
}
